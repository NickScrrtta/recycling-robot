...
char checkColour(){
  ...
  if(white_f < 400 && red_f < 1200){ 
    for(int i = 0; i < 3; i++){ turnOnLEDs(); delay(200); turnOffLEDs(); delay(200); } delay(1000); colour = 'w'; }
    
  else if((green_f < blue_f && green_f > red_f) || (green_f < red_f && green_f < blue_f) || (green_f == red_f && green_f < blue_f)){ 
    shiftWrite(GREEN_LED, HIGH); delay(1000); colour = 'g'; }
    
  else if(blue_f < green_f && blue_f < red_f){ 
    shiftWrite(BLUE_LED, HIGH); delay(1000); colour = 'b'; }
    
  else if(red_f < green_f && red_f < blue_f){ 
    shiftWrite(RED_LED, HIGH); delay(1000); colour = 'r'; }
    
  else{ 
    for(int i = 0; i < 3; i++){ shiftWrite(RED_LED, HIGH); delay(100); shiftWrite(GREEN_LED, HIGH); delay(100); shiftWrite(BLUE_LED, HIGH); delay(100); turnOffLEDs(); delay(100); } colour = 'e'; } 
  
  turnOffLEDs();
  return colour;
}

void turnOnLEDs(){ shiftWrite(RED_LED, HIGH); shiftWrite(GREEN_LED, HIGH); shiftWrite(BLUE_LED, HIGH); }

void turnOffLEDs(){ shiftWrite(RED_LED, LOW); shiftWrite(GREEN_LED, LOW); shiftWrite(BLUE_LED, LOW); }
