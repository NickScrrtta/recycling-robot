...
void takeInventory()
{
  bitSet(cubes[1], 0); bitSet(cubes[2], 1); bitSet(cubes[3], 0); bitSet(cubes[3], 1);
   
  for(int j = 0; j < 4; j++){
    Serial.print("Rotation "); Serial.println(j);
      delay(800);  
      if(checkMagnetic()){ if(j == 3){ bitSet(cubes[0], 2); bitSet(cubes[0], 4); } else{ bitSet(cubes[j+1], 2); bitSet(cubes[j+1], 4); } } 
      if(checkMetal()){ if(j < 2){ bitSet(cubes[j+2], 3); bitSet(cubes[j+2], 4); } else{ bitSet(cubes[j-2], 3); bitSet(cubes[j-2], 4); } }  
      if(checkClear()){ if(j == 0){ bitSet(cubes[3], 2); bitSet(cubes[3], 3); bitSet(cubes[3], 4); } else{ bitSet(cubes[j-1], 2); bitSet(cubes[j-1], 3); bitSet(cubes[j-1], 4); } }
    Serial.println(" "); 

    for(int i = 0; i < 4; i++){printCubeBits(i);}
    Serial.println(" ");
    rotate(90);
  }

  for(int k = 0; k < 4; k++){ if(bitRead(cubes[k], 2) == 0 && bitRead(cubes[k], 3) == 0) bitSet(cubes[k], 4); }
...
{  
  ...
  if(hallEffectValue < HALL_EFFECT_LOWER_BOUND || hallEffectValue > HALL_EFFECT_UPPER_BOUND){
    Serial.print("checkMagnetic PASSED, hallEffectValue = "); Serial.println(hallEffectValue);
    return TRUE;
  }
  else{
    Serial.print("checkMagnetic FAILED, hallEffectValue = "); Serial.println(hallEffectValue);
    return FALSE;
  }
}
...
bool checkClear()
{
  ...
  if (photoResistorValue > PHOTO_RESISTOR_MIN){
    Serial.print("checkClear PASSED, photoResistorValue = "); Serial.println(photoResistorValue);
    return TRUE; 
  }
  else{
    Serial.print("checkClear FAILED, photoResistorValue = "); Serial.println(photoResistorValue);
    return FALSE;
  }
}
...
void pushCube(){ servo.write(50); delay(100); servo.write(10); delay(200); servo.write(50); delay(100); }

int findCube(int bit3, int bit2){ for(int i = 0; i < 4; i++){ if(bitRead(cubes[i], 3) == bit3 && bitRead(cubes[i], 2) == bit2 && bitRead(cubes[i], 4) == 1) return i; } return -1; }

// rotates the platform (deg) number of degrees (0, 90, 180, or 270) counterclockwise
void rotate(int deg){ for(int i = 0; i < (deg/90); i++){ stepperMotor.setSpeed(STEPPER_SPEED); stepperMotor.step(-STEPPER_STEPS); cube_at_door++; } cube_at_door = cube_at_door%4; }

void printCubeBits(int i){ for(int j = 7; j >= 0; j--){ Serial.print(bitRead(cubes[i], j)); } Serial.println(" "); }

void initServo(){ servo.attach(SERVO_PIN); servo.write(50); } // put door to starting position

void initCubes(){ for(int i = 0; i < 4; i++){ for(int j = 0; j < 8; j++){ bitClear(cubes[i], j); } } }
